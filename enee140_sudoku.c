 ...
 */
    ...
    for (j = 0; j < 9; j++) { //Rows
        for (i = 0; i < 9; i++) { //Columns
            if(board[i][j] < 0 || board[i][j] > 9) { //Checks each value in the matrix if it is less than 0 or more than 9;
                return 0; // Repeated value has been found.
            }
        }
    }
 ...
 */
        for (i = 0; i < 9; i++) {
            for (j = 0; j < 9; j++) {
                if ((j + 1) % 3 == 0 && (i + 3) % 3 == 0) { // This stops the loop if it reaches the upper right hand corner of a box.
...
int boardcopy[9][9]; // Copy of board.
...
// Reflecting indices: j (column index) becomes i (row index).
    for(i = 0; i < 9; i++) {
        for (j = 0; j < 9; j++) {
            board[i][j] = boardcopy[j][i];
            board[j][i] = boardcopy[i][j];
        }
    }
}
...
// Reflecting indices: j (column index) becomes i (row index).
    for(i = 0; i < 9; i++) {
        for (j = 0; j < 9; j++) {
            board[i][j] = boardcopy[8-j][8-i];
            board[8-j][8-i] = boardcopy[i][j];
        }
    }
}
